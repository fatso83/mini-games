...
export type ClientCommand =
  ...
  | { readonly cmd: 'start' }
export interface PublicPlayer { readonly id: string; readonly name: string; readonly status: 'active' | 'out'; readonly connected: boolean; readonly score: number; readonly body: readonly Position[] }
export interface PublicSession { readonly code: string; readonly status: 'lobby' | 'running' | 'results' | 'finished'; readonly hostId: string | null; readonly winnerId: string | null; readonly level: number; readonly tick: number; readonly config: { readonly width: number; readonly height: number; readonly tickIntervalMs: number }; readonly obstacles: readonly Position[]; readonly players: readonly PublicPlayer[]; readonly food: Position | null }
...
export function validDisplayName(value: unknown): string | null { if (typeof value !== 'string') return null; const name = value.trim(); return /^[a-z0-9_ -]{1,20}$/i.test(name) ? name : null }
export function parseClientCommand(value: string | ArrayBuffer): ClientCommand | null {
  try { const parsed: unknown = JSON.parse(typeof value === 'string' ? value : new TextDecoder().decode(value)); if (!parsed || typeof parsed !== 'object' || !('cmd' in parsed)) return null; const candidate = parsed as { cmd?: unknown; name?: unknown; direction?: unknown }; if (candidate.cmd === 'join' && typeof candidate.name === 'string') return { cmd: 'join', name: candidate.name }; if (candidate.cmd === 'start') return { cmd: 'start' }; if (candidate.cmd === 'direction' && (candidate.direction === 'up' || candidate.direction === 'down' || candidate.direction === 'left' || candidate.direction === 'right')) return { cmd: 'direction', direction: candidate.direction }; } catch { /* malformed */ } return null
}
