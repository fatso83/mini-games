...
export type ClientCommand =
  ...
  | { readonly cmd: 'resume'; readonly token: string }
...
export interface PublicPause { readonly reason: 'connection_lost'; readonly pausedAt: number; readonly expiresAt: number; readonly waitingPlayerIds: readonly string[] }
export interface PublicSession { readonly code: string; readonly isPublic: boolean; readonly status: 'lobby' | 'countdown' | 'running' | 'paused' | 'results' | 'finished'; readonly countdown: PublicCountdown | null; readonly pause: PublicPause | null; readonly hostId: string | null; readonly winnerId: string | null; readonly level: number; readonly tick: number; readonly config: { readonly width: number; readonly height: number; readonly tickIntervalMs: number }; readonly obstacles: readonly Position[]; readonly players: readonly PublicPlayer[]; readonly food: Position | null }
export interface SnapshotEvent { readonly cmd: 'snapshot'; readonly seq: number; readonly selfPlayerId: string | undefined; readonly resumeToken?: string; readonly state: PublicSession; readonly players: readonly PublicPlayer[] }
...
export function parseClientCommand(value: string | ArrayBuffer): ClientCommand | null {
  try { const parsed: unknown = JSON.parse(typeof value === 'string' ? value : new TextDecoder().decode(value)); if (!parsed || typeof parsed !== 'object' || !('cmd' in parsed)) return null; const candidate = parsed as { cmd?: unknown; name?: unknown; token?: unknown; direction?: unknown }; if (candidate.cmd === 'join' && typeof candidate.name === 'string') return { cmd: 'join', name: candidate.name }; if (candidate.cmd === 'resume' && typeof candidate.token === 'string' && candidate.token.length >= 16 && candidate.token.length <= 128) return { cmd: 'resume', token: candidate.token }; if (candidate.cmd === 'start') return { cmd: 'start' }; if (candidate.cmd === 'restart') return { cmd: 'restart' }; if (candidate.cmd === 'direction' && (candidate.direction === 'up' || candidate.direction === 'down' || candidate.direction === 'left' || candidate.direction === 'right')) return { cmd: 'direction', direction: candidate.direction }; } catch { /* malformed */ } return null
}
