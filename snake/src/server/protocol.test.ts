...
describe('session protocol utilities', () => {
  ...
  it('parses the client-facing cmd protocol', () => { expect(parseClientCommand('{"cmd":"join","name":"Alice"}')).toEqual({ cmd: 'join', name: 'Alice' }); expect(parseClientCommand('{"cmd":"resume","token":"5b13a879-9d8b-4f15-bc51-2f7d4b6918e2"}')).toEqual({ cmd: 'resume', token: '5b13a879-9d8b-4f15-bc51-2f7d4b6918e2' }); expect(parseClientCommand('{"cmd":"start"}')).toEqual({ cmd: 'start' }); expect(parseClientCommand('{"cmd":"restart"}')).toEqual({ cmd: 'restart' }); expect(parseClientCommand('{"type":"start_game"}')).toBeNull() })
