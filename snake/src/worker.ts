...
import { SESSION_ALPHABET, createSessionCode, parseClientCommand, validDisplayName, type ClientCommand, type PublicCountdown, type PublicPlayer, type PublicSession, type ServerEvent } from './server/protocol'
...
export interface Env { readonly ASSETS: FetcherLike; readonly GAME_ROOMS: DurableObjectNamespaceLike; readonly SERVER_URL?: string }
...
interface RoomState { code: string; status: RoomStatus; countdown: PublicCountdown | null; hostId: string | null; level: number; tick: number; seq: number; food: Position | null; players: RoomPlayer[]; createdAt: number; resultsAt: number | null; winnerId: string | null }
...
export class GameRoom {
  ...
  async alarm(): Promise<void> {
    ...
    if (room.status === 'finished' && room.resultsAt !== null) { if (now - room.resultsAt >= 180_000) { await this.state.storage.deleteAll(); this.room = null; return } await this.state.storage.setAlarm(room.resultsAt + 180_000); return }
...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/api/sessions' && request.method === 'POST') {
    ...
    return json({ code, socket: `/api/sessions/${code}/socket`, shareUrlBase: effectiveBase }, 201)
  }
