...
export interface ClientProtocolState {
  ...
  readonly phase: 'idle' | 'lobby' | 'running' | 'ended' | 'error'
...
export function reduceServerMessage(state: ClientProtocolState, message: ServerMessage): ClientProtocolState {
  ...
  const statusPhase = (value: unknown): ClientProtocolState['phase'] => value === 'running' ? 'running' : value === 'results' || value === 'finished' ? 'ended' : 'lobby'
